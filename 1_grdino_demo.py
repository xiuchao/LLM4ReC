...
def plot_boxes_to_image(image_pil, tgt):
    ...
    for box, label in zip(boxes, labels):
        ...
        if hasattr(font, "getbbox"):
            txtbox = draw.textbbox((x0, y0), str(label), font)
        else:
...
if __name__ == "__main__":
    ...
    image_pil, image = load_image(image_path) #(4032, 3024), [3, 800, 1066]
    ...
    size = image_pil.size
    pred_dict = {
        "boxes": boxes_filt,         # norm
        "size": [size[1], size[0]],  # H,W
        "labels": pred_phrases,
    }
    # import ipdb; ipdb.set_trace()
    image_with_box = plot_boxes_to_image(image_pil, pred_dict)[0]
    image_with_box.save(os.path.join(output_dir, "gdino_"+text_prompt+".jpg"))